<div class="flex flex-col gap-6">
    <div class="flex justify-between items-center">
        ...
        <flux:button variant="primary" icon="plus" wire:click="create">{{ __('New Policy') }}</flux:button>
    </div>
    ...
    <flux:card class="p-0! overflow-hidden">
        <flux:table>
            <flux:table.columns>
                ...
                <flux:table.column sortable :sorted="$sortCol === 'limit_days'" :direction="$sortAsc ? 'asc' : 'desc'" wire:click="sortBy('limit_days')">{{ __('Limit Days') }}</flux:table.column>
                <flux:table.column sortable :sorted="$sortCol === 'period_days'" :direction="$sortAsc ? 'asc' : 'desc'" wire:click="sortBy('period_days')">{{ __('Period Days') }}</flux:table.column>
            ...
            <flux:table.rows>
                @foreach($policies as $policy)
                    <flux:table.row :key="$policy->id">
                        <flux:table.cell class="font-medium">{{ $policy->name }}</flux:table.cell>
                        <flux:table.cell>{{ $policy->type->label() }}</flux:table.cell>
                        <flux:table.cell>{{ $policy->limit_days ?: '-' }}</flux:table.cell>
                        <flux:table.cell>{{ $policy->period_days ?: '-' }}</flux:table.cell>
                        <flux:table.cell>
                            <flux:dropdown>
    ...
    <!-- Edit/Create Modal -->
    <flux:modal wire:model="showModal" class="min-w-150">
        <div class="space-y-6">
            <div>
