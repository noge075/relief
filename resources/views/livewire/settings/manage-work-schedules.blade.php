    ...
    <flux:card class="p-0! overflow-hidden">
        <flux:table>
            <flux:table.columns>
                ...
                <flux:table.column class="hidden md:table-cell">{{ __('Weekly Hours') }}</flux:table.column>
                <flux:table.column class="hidden md:table-cell">{{ __('Work Time') }}</flux:table.column>
                <flux:table.column class="hidden md:table-cell">{{ __('Weekly Pattern') }}</flux:table.column>
                    ...
                    <flux:table.row :key="$schedule->id">
                        <flux:table.cell>
                            <div class="flex flex-col">
                                <span class="font-medium text-zinc-900 dark:text-zinc-100">{{ $schedule->name }}</span>

                                <div class="md:hidden mt-2 space-y-2">
                                    <div class="text-xs text-zinc-500 flex items-center gap-2">
                                        @if($schedule->start_time && $schedule->end_time)
                                            <span class="bg-zinc-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded text-zinc-600 dark:text-zinc-300">
                                                {{ \Carbon\Carbon::parse($schedule->start_time)->format('H:i') }} - {{ \Carbon\Carbon::parse($schedule->end_time)->format('H:i') }}
                                            </span>
                                        @endif
                                        <span class="font-semibold text-zinc-700 dark:text-zinc-300">
                                            {{ array_sum($schedule->weekly_pattern) }} {{ __('h') }} / {{ __('week') }}
                                        </span>
                                    </div>
                        ...
                        <flux:table.cell>
                            <div class="flex md:hidden gap-2">
                                <button
                                        ...
                                        class="w-8 h-8 flex items-center justify-center rounded-full bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400 active:scale-95 transition-transform"
                                >
                            ...
                            <div class="hidden md:block">
                                <flux:dropdown>
                                    ...
                                    <flux:menu>
                                        ...
                                        <flux:menu.separator />
                                        <flux:menu.item wire:click="delete({{ $schedule->id }})" icon="trash" variant="danger" wire:confirm="{{ __('Are you sure?') }}">{{ __('Delete') }}</flux:menu.item>
                                    </flux:menu>
                                </flux:dropdown>
                            </div>
                        </flux:table.cell>
    ...
    <flux:modal wire:model="showModal" class="w-full sm:w-140">
        <div class="space-y-6">
            ...
            <div class="grid gap-4">
                ...
                <div class="space-y-2">
                    <flux:label>{{ __('Weekly Pattern (Hours)') }}</flux:label>
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4 bg-zinc-50 dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700">
                        @foreach(['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'] as $day)
