<flux:modal wire:model="showModal" class="w-full sm:w-11/12 max-w-5xl max-h-[90vh] overflow-y-auto">
    <div class="space-y-6" x-data="{ tab: 'basic' }">
        ...
        <div class="flex border-b border-zinc-200 dark:border-zinc-700 overflow-x-auto pb-1 -mx-4 px-4 sm:mx-0 sm:px-0">
            <button @click="tab = 'basic'" :class="tab === 'basic' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-zinc-500 hover:text-zinc-700 hover:border-zinc-300'" class="px-4 py-2 border-b-2 font-medium text-sm transition-colors whitespace-nowrap">
                {{ __('Basic Info') }}
            </button>
            <button @click="tab = 'personal'" :class="tab === 'personal' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-zinc-500 hover:text-zinc-700 hover:border-zinc-300'" class="px-4 py-2 border-b-2 font-medium text-sm transition-colors whitespace-nowrap">
                {{ __('Personal Details') }}
            </button>
            <button @click="tab = 'permissions'" :class="tab === 'permissions' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-zinc-500 hover:text-zinc-700 hover:border-zinc-300'" class="px-4 py-2 border-b-2 font-medium text-sm transition-colors whitespace-nowrap">
                {{ __('Permissions') }}
            </button>
            @if($isEditing)
                <button @click="tab = 'documents'" :class="tab === 'documents' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-zinc-500 hover:text-zinc-700 hover:border-zinc-300'" class="px-4 py-2 border-b-2 font-medium text-sm transition-colors whitespace-nowrap">
                    {{ __('Documents') }}
                </button>
            @endif
        </div>
        ...
        <div class="min-h-80">
            ...
            <div x-show="tab === 'permissions'" class="space-y-4" style="display: none;">
                ...
                <div class="mt-2 space-y-4 max-h-96 overflow-y-auto border rounded-lg p-4 bg-zinc-50 dark:bg-zinc-900">
            ...
            @if($isEditing)
                <div x-show="tab === 'documents'" class="space-y-4" style="display: none;">
                    @can(\App\Enums\PermissionType::MANAGE_USER_DOCUMENTS->value)
                        <div class="space-y-2">
                            ...
                            <div class="flex justify-end mt-2">
                                <flux:button wire:click="uploadDocument" variant="primary" size="sm" :disabled="!$documentUpload" class="w-full sm:w-auto">{{ __('Upload') }}</flux:button>
                            </div>
                        </div>
                        <flux:separator />
                    @endcan
                    ...
                    @if($userDocuments && $userDocuments->isNotEmpty())
                        <div class="space-y-2">
                            @foreach($userDocuments as $media)
                                <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-2 p-3 bg-zinc-50 dark:bg-zinc-800 rounded border border-zinc-200 dark:border-zinc-700">
                    ...
                    @else
                        <p class="text-sm text-zinc-500 italic">{{ __('No documents found.') }}</p>
                    @endif
                </div>
            @endif
        </div>

        <div class="flex flex-col-reverse sm:flex-row justify-end gap-2 pt-4 border-t border-zinc-200 dark:border-zinc-700">
            <flux:button variant="ghost" wire:click="$set('showModal', false)" class="w-full sm:w-auto">{{ __('Cancel') }}</flux:button>
            <flux:button variant="primary" wire:click="save" class="w-full sm:w-auto">{{ __('Save') }}</flux:button>
        </div>
    </div>
</flux:modal>